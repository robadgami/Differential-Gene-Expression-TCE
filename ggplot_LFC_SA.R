...
ggplot(SA_F22v1314_LFC_1dpi, aes(y=`log2FoldChange-F22v1314`, x=`log2FoldChange-SAvOA`, color=annotated, label=gene)) + geom_point(size=1.5) + theme_bw() +
  xlab('LFC for F22-infected Santiago vs Oakley') + ylab('LFC for Santiago infected by F22 vs 13/14') +
  geom_vline(xintercept=0, color='dark grey') + geom_hline(yintercept=0, color='dark grey') + ggtitle('1 dpi') +
  geom_text(aes(label=ifelse(`log2FoldChange-F22v1314`> 0.5,as.character(gene),'')),hjust=0.6,vjust=1.5, size=3, angle=10) +
  geom_text(aes(label=ifelse(`log2FoldChange-F22v1314`< -0.5,as.character(gene),'')),hjust=0.6,vjust=1.5, size=3, angle=10)

SA_F22v1314_LFC_3dpi <- SA_F22v1314_LFC[SA_F22v1314_LFC$dpi == '3dpi',]
View(SA_F22v1314_LFC_3dpi)
ggplot(SA_F22v1314_LFC_3dpi, aes(y=`log2FoldChange-F22v1314`, x=`log2FoldChange-SAvOA`, color=annotated, label=gene)) + geom_point(size=1.5) + theme_bw() +
  xlab('LFC for F22-infected Santiago vs Oakley') + ylab('LFC for Santiago infected by F22 vs 13/14') +
  ...
  geom_text(aes(label=ifelse(`log2FoldChange-SAvOA`> -1,as.character(gene),'')),hjust=0.5,vjust=1.5, size=3, angle=90)
